...
#    print exptime
    sf1.make_dark(band, exptime, overwrite=False)
    sf1.make_dark(band, 5.0, overwrite=False)
    
    sf2 = sf.SacraFile(DT_FLAT)
   #    band="i"
    exptime_flat = 5.0
    sf2.make_flat(band, exptime_flat, MOD_MIMG, overwrite=False)

    sf3 = sf.SacraFile(DT_OBJ)
    if band == 'z':
        sf3.darksub_flatten(objname, band, MOD_MIMG, flip=True)
    else:
        sf3.darksub_flatten(objname, band, MOD_MIMG)

#    fi = sf.FitsInfo(band=band, datatype=DT_FLAT)
#    lst=sf.get_fn_list('flat', fi)
#    print(lst)
#    fn_list=sf.getFlatList( band )
#    sf.flatCombine(band, fn_list)
#    liststr=sf.get_fns_str(fn_list)
#    print(liststr)
#    print(sf.get_dark_fn(band, 5000.0))
#    print(sf.get_flat_fn(band, "img"))
