...
class SacraFile(object):
    ...
    def add_fits_histories(self, fn, history_strs):
        sfts = SacraFits(fn)
        for line in history_strs.splitlines():
            sfts.add_history(line)
        sfts.update_fits_file()
        sfts.close()
    ...
    def wcs_reproject(self, lst):
        '''WCS reproject by reproject module.
        '''
        reproj = []
        hdul = fits.open(lst[0])
        for fn in lst:
            fn_out = self.get_fn_with_sub_extention(fn, '_rpj')
            print('WCS reprojecting: {} to {}'.format(fn, fn_out))
            array_new, footprint = reproject.reproject_exact(fn, hdul[0].header)
            hdul_out = fits.open(fn)
            hdul_out[0].data = array_new
            hdul_out.writeto(fn_out)
            hdul_out.close()
            self.add_fits_histories(fn_out, '# wcs_reproject: {}\n{}->{}'.format(datetime.datetime.utcnow().strftime('%c'), fn, fn_out))
            reproj.append(fn_out)
        hdul.close()
        return reproj  # File list of reprojected files.
    

    def img_combine(self, lst, out_fn, reject=False, wcs_rp=False,
        ...
        if wcs_rp == True:
            print('Reproject WCS.')
            lst = self.wcs_reproject(lst)
        if (os.path.isfile(out_fn)):
            print('File %s exists. Abort.' % (out_fn))
            exit(1)
        try:
            if (reject == True):
                ccdproc.combine(lst, out_fn, method='average', sigma_clip=True,
                ...
                #               zero=zero)
                ccdproc.combine(lst, out_fn, method='average', sigma_clip=False)
                #iraf.imcombine("@%s" % tl_f.name, out_fn, offsets=offsets,
                #               zero=zeroopt)
